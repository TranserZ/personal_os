...
/*
	...
	cross-compiler prefix는 '내 PC에서 다른 기기(다른 프로세스를 가진)용
	...
	위한 source code를 x86 PC에서 컴파일한다) 즉, 다른 프로세스를 탑재했다. 
...
/*
	...
	C_FILES = $(wildcard $(SRC_DIR)/*.c)
		...
		Ex ... SRC_DIR=src이고 src/에 a.c와 b.c가 있다면: C_FILES = src/a.c src/b.c

	ASM_FILES = $(wildcard $(SRC_DIR)/*.S)
		SRC_DIR로 설정해 둔 디렉토리 안 .S로 끝나는 파일들을 전부 나열.
		Ex ... SRC_DIR=src이고 src/에 x.S와 y.S가 있다면: ASM_FILES = src/x.S src/y.S
